...
def _build_parser():
    ...
    ranker.add_argument('-s', '--sort', default='ca_pred',
                        choices=['ca_pred', 'residual', 'above_league'],
                        help='ca_pred: best players; residual: most underrated '
                             'by FM (out-of-fold, so it will not equal '
                             'CA_pred - CA); above_league: best relative to '
                             'their league')
...
def cmd_convert(args):
    ...
    written = html_import.convert_all(
...
def cmd_compare(args):
    ...
    """
    configs = [
        ('identity target, no interactions', 'identity', False),
        ('log target, no interactions', 'log', False),
        ('identity + league interactions', 'identity', True),
        ('log target + league interactions', 'log', True),
    ]
    print(f"{'config':<34}{'cv_r2':>8}{'test_r2':>9}{'MAE(CA)':>9}"
          f"{'resid~league':>14}{'feats':>7}")
    print('-' * 81)
    rows = []
    for label, transform, interactions in configs:
        try:
        ...
        print(f"{label:<34}{m['cv_r2_mean']:>8.3f}{m['test_r2']:>9.3f}"
    ...
    _, label, transform, interactions = min(rows)
...
def cmd_rank(args):
    ...
    sort_col = {'ca_pred': ca_model.PRED_COL,
                'residual': ca_model.RESIDUAL_COL,
    ...
    columns = [c for c in (NAME_COL, 'Nation', ca_model.TARGET, ca_model.PRED_COL,
                           ca_model.RESIDUAL_COL, 'pred_above_league', VALUE_COL)
...
def _plot(position, scored, ranked):
    ...
    plot_shortlist(plottable, ranked,
                   ...
                   reference=float(plottable[ca_model.PRED_COL].median()),
